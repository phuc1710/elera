...
part 'my_ongoing_course_event.dart';
part 'my_ongoing_course_state.dart';

@injectable
class MyOngoingCourseBloc
    extends Bloc<MyOngoingCourseEvent, MyOngoingCourseState> {
  MyOngoingCourseBloc(this.myCourseDetailFetchUseCase)
      : super(MyOngoingCourseInitial()) {
    on<MyOngoingCourseFetched>(_onMyOngoingCourseFetched);
  }

  final MyCourseDetailFetchUseCase myCourseDetailFetchUseCase;

  Future<void> _onMyOngoingCourseFetched(
    MyOngoingCourseFetched event,
    Emitter<MyOngoingCourseState> emit,
  ) async {
    emit(MyOngoingCourseFetchInProgress());
    ...
    if (dataState is DataSuccess) {
      ...
      if (dataState.data?.code == ErrorCode.success)
        emit(MyOngoingCourseFetchSuccess(res?.data));
      else
        emit(
          MyOngoingCourseFetchFailure(
